...
from optclient.solver_utils.ortools.solver import Solver, OptSense
...
def create_sample_request():
    ...
    xp1 = LinExpr([x1, x2, x3], [1.0, 1.0, 1.0], name="exp1")
    xp2 = LinExpr([x1, x2, x3], [2.5, 1.0, 1.25], name="c2")
    xp3 = LinExpr([x1, x2, x3], [1.0, 1.0, 3.0], name="c3")

    c1 = LinConstraint(expr=xp1, rhs=100.0, name="c1", sense=ConstrSense.leq)
    c2 = LinConstraint(expr=xp2, rhs=150.0, name="c2", sense=ConstrSense.leq)
    c3 = LinConstraint(expr=xp3, rhs=150.0, name="c3", sense=ConstrSense.leq)
...
def create_sample_request2():
    ...
    xp1 = LinExpr([x1, x2, x3], [1.0, 1.0, 1.0], name="exp1")
    xp2 = LinExpr([xp1, x1, x3], [1 , 1.5, 0.25], name="c2")
    xp3 = LinExpr([xp1, x3], [1.0, 2.0], name="c3")
